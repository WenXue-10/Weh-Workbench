...
function saveDecisionSettings(){
  ...
  saveDecision(data);
  updateDecStyleHint();
  toast("设置已保存："+data.identity+" · "+data.stage+" · "+data.depth+"轮拷问 · "+(DEC_STYLE_TEXT[data.style]||DEC_STYLE_TEXT.harsh));
}
...
function buildDecisionMessages(data){
  ...
  if(data.style === "senior"){
    sys = "你是一位资深决策顾问，服务对象是一位中文用户。任务：先指出他这段回答里最关键的薄弱环节，再给可执行的建议，最后追问一个真正决定成败的问题。"
  ...
  var inst = (left > 1)
    ? "【本轮要求】先用不超过 80 字指出我这段回答里最致命的漏洞，再提一个最难回答的追问（不超过 40 字）。\n只输出一个 JSON 对象，不要 markdown 代码块、不要任何解释文字：\n{\"content\":\"你的点评\",\"question\":\"你的追问\"}"
    : "【本轮要求】这是最后一轮，先用不超过 80 字给出最终点评，question 字段留空字符串。\n只输出一个 JSON 对象，不要 markdown 代码块、不要任何解释文字：\n{\"content\":\"你的点评\",\"question\":\"\"}";
...
function generateDecisionResponseAI(data){
  ...
  return aiChat(cfg, buildDecisionMessages(data), 45).then(function(res){
    ...
    return { content: _aiStr(o.content), question: _aiStr(o.question) || null,
...
function buildDecisionScoreMessages(data){
  ...
  var inst = "对话记录：\n" + linesTxt + "\n\n"
           ...
           + "{\"score\":7,\"fatal\":\"一句话指出致命伤\",\"fix\":\"一句话给出该怎么改\"}";
...
function parseReportAI(txt){
  ...
  return { opening: _aiStr(o.opening), highlights: hs.slice(0, 5), questions: qs.slice(0, 3), pitfalls: ps.slice(0, 3) };
}
