...
def status_key(t):
    ...
    if "已失效" in t: return "invalid"
    if "已背调" in t: return "done"
    if "offer" in low: return "offer"
    if "待投递" in t or "已写简历" in t or "定制简历" in t: return "pending"
    if "已投递" in t: return "applied"
    if "面试" in t: return "interview"
    if "主动放弃" in t: return "abandoned"
    if "红线" in t or "淘汰" in t: return "redline"
    if "已挂" in t: return "rejected"
    if "待确认" in t or "待补充" in t or "待核实" in t or "待背调" in t or "待人工核实" in t or "人工核实" in t: return "warn"
